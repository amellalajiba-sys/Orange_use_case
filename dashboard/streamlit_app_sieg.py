<<<<<<< HEAD
"""
Innovation Radar -- Streamlit dashboard.
...
import pandas as pd
import re  # Sieg 26/8 -- for the published_date format fix in section 16
...
# Sieg 25/8 -- same idea (wide layout, Orange branding), plus a dedicated
# icon and a dark/orange CSS theme applied globally, because the polar
# radar chart added reads much better on a dark background.
st.set_page_config(page_title="Innovation Radar — Orange Business", layout="wide", page_icon="📡")
...
section[data-testid="stSidebar"] { background-color: var(--radar-panel); border-right: 1px solid #262b36; }
h1, h2, h3 { color: var(--radar-text) !important; }
.stMetric { background-color: var(--radar-panel); border: 1px solid #262b36; border-radius: 10px; padding: 10px 14px; }
[data-testid="stMetricValue"] { color: var(--radar-orange) !important; }
/* Sieg 25/8 -- st.metric labels ("Attractiveness", "Right to win", "Urgency")
...
div[data-testid="stDataFrame"] { border: 1px solid #262b36; border-radius: 8px; }
</style>
"""
...
# ============================================================
# 3. SIDEBAR ROLE + LOADING THE DATA
# ============================================================


# Sieg 25/8 -- role picked first (needed below to pre-filter Presales to
# deliverable-soon OS), then load. Empty-state message now distinguishes
# "nothing ingested yet" from "ingested but not scored yet" and tells you
# the exact next pipeline command instead of one generic error.
...
# Sieg 25/8 -- moved after the filters since the caption
# needs role/sort_by/total_os_count, which don't exist yet at 
# original position. Same spirit: one-line title + one-line context.
st.title("📡 Innovation Radar")
st.caption(f"{role} · ordered by {sort_by.lower()} · {total_os_count} opportunity spaces tracked")
...
with col2:
    ...
    st.metric("⭐ Strong opportunities", f"{strong_count} / {len(filtered)}",
              help="Attractiveness >= 7 AND Right-to-win >= 7 -- same threshold "
                   "as the quadrant message in the detail panel below.")
...
with col5:
    ...
    st.caption(f"⭐ Strong: {strong}  \n⚠️ Needs capability: {needs_capability}  \n"
               f"💡 Moderate market: {moderate_market}  \n📉 Low both: {low_both}")
...
with col_radar:
    ...
    with tab_radar:
        ...
        fig.add_trace(go.Scatterpolar(
            ...
            marker=dict(
                ...
                cmin=0, cmax=10,
                colorbar=dict(
                    title=dict(text="Right to win", font=dict(color="#e6e6e6")),
        ...
        fig.update_layout(
            polar=dict(
                ...
                angularaxis=dict(
                    ...
                    direction="clockwise",
                    gridcolor="#262b36",
                    tickfont=dict(color="#e6e6e6", size=11),
                ),
            ),
    ...
    with tab_bubble:
        ...
        fig_bubble.update_layout(
            xaxis=dict(title="Attractiveness", range=[0, 10.5], gridcolor="#262b36",
                       tickfont=dict(color="#8a8f98"), title_font=dict(color="#e6e6e6")),
            yaxis=dict(title="Right to win", range=[0, 10.5], gridcolor="#262b36",
                       tickfont=dict(color="#8a8f98"), title_font=dict(color="#e6e6e6")),
            paper_bgcolor="#0e1117", plot_bgcolor="#161b24",
            font=dict(color="#e6e6e6"),
            legend=dict(font=dict(color="#e6e6e6"), title=dict(text="Vertical")),
            height=620,
...
with col_detail:
    ...
    picked_label = st.selectbox("Opportunity space", labels, index=default_index)
    ...
    ##     with score_col1:
    ##         st.metric("Attractiveness", f"{selected['Attractiveness']:.2f}/10")
    ##     with score_col2:
    ##         st.metric("Right-to-Win", f"{selected['Right to Win']:.2f}/10")
...
with tab_score:
    ...
    fig_breakdown.update_layout(
        polar=dict(
            bgcolor="#161b24",  # --radar-panel
            radialaxis=dict(visible=True, range=[0, 10], color="#8a8f98"),
            angularaxis=dict(color="#e6e6e6"),
        ),
        paper_bgcolor="#0e1117",  # --radar-bg
        showlegend=False,
...
if __name__ == "__main__":
    run_full_refresh()
>>>>>>> e3bd0a762b5540a9c89feffd2fdfc529a275f765
